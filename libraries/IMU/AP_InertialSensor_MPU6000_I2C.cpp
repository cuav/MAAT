 ...
 */
//FS_SEL = 2
const float AP_InertialSensor_MPU6000_I2C::_gyro_scale = (0.0174533f / 16.25f);//(0.0174532 / 16.4);
...
/*    _gyro.x = 0;
    ...
    _accel_data_sign[2]  = -1;
		*/
		...
		_gyro_data_index[0]  = 5;//4;		// Y
    _gyro_data_index[1]  = 4;//5;		// X
    _gyro_data_index[2]  = 6;		// Z
    _gyro_data_sign[0]   = 1;	// Y
    _gyro_data_sign[1]   = 1;		// X
    _gyro_data_sign[2]   = -1;	// -Z

    _accel_data_index[0] = 1;//0;		//
    _accel_data_index[1] = 0;//1;
    _accel_data_index[2] = 2;
    _accel_data_sign[0]  = 1;
    _accel_data_sign[1]  = 1;
    _accel_data_sign[2]  = -1;
...
bool AP_InertialSensor_MPU6000_I2C::init( /*AP_PeriodicProcess * scheduler */)
{
...
bool AP_InertialSensor_MPU6000_I2C::update( void )
{
    ...
    gyro[0]/*_gyro.x*/ = _gyro_scale * _gyro_data_sign[0] * sum[_gyro_data_index[0]] * count_scale;
    gyro[1]/*_gyro.y*/ = _gyro_scale * _gyro_data_sign[1] * sum[_gyro_data_index[1]] * count_scale;
    gyro[2]/*_gyro.z*/ = _gyro_scale * _gyro_data_sign[2] * sum[_gyro_data_index[2]] * count_scale;

    accel[0]/*_accel.x*/ = _accel_scale * _accel_data_sign[0] * sum[_accel_data_index[0]] * count_scale;
    accel[1]/*_accel.y*/ = _accel_scale * _accel_data_sign[1] * sum[_accel_data_index[1]] * count_scale;
    accel[2]/*_accel.z*/ = _accel_scale * _accel_data_sign[2] * sum[_accel_data_index[2]] * count_scale;
...
/*
float AP_InertialSensor_MPU6000_I2C::gx()
{
    return _gyro.x;
}
float AP_InertialSensor_MPU6000_I2C::gy()
{
    return _gyro.y;
}
float AP_InertialSensor_MPU6000_I2C::gz()
{
    return _gyro.z;
}
...
float AP_InertialSensor_MPU6000_I2C::ax()
{
    return _accel.x;
}
float AP_InertialSensor_MPU6000_I2C::ay()
{
    return _accel.y;
}
float AP_InertialSensor_MPU6000_I2C::az()
{
    return _accel.z;
}
...
void AP_InertialSensor_MPU6000_I2C::hardware_init()
{
    ...
    delay(1);
    if (I2c.write(mpu_addr, MPUREG_GYRO_CONFIG, BITS_FS_2000DPS/*BITS_FS_1000DPS*/) != 0)   // Gyro scale 2000º/s
    {
    	debug("ck5\r\n");
        return;
    }
