...
public:
    ...
    ///
    virtual void calculate_heading(float roll, float pitch);

    /// Calculate the tilt-compensated heading_ variables.
    ///
    /// @param dcm_matrix			The current orientation rotation matrix
    ///
    virtual void calculate_heading(const Matrix3f &dcm_matrix);
    ...
    virtual void calibrate(void (*_delay)(uint32_t) = delay);
    virtual void quit_calibrate(){_in_calibrating = false;}
    virtual void reset_calibration(){memset(_offset, 0, sizeof(_offset));};
    /// Returns the current offset values
    ///
    /// @returns                    The current compass offsets.
    ///
    virtual float* get_offsets();
...
protected:
    ...
    uint8_t* 	_setting_rom;
    bool 		_in_calibrating;
